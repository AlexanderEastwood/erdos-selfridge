...
v5 = importlib.import_module(os.environ.get("ES_ENGINE", "gpu_sieve_v8_masks"))
...
def engine_revision() -> dict[str, str]:
    paths = ('sieve/gpu_sieve_v8.py', 'sieve/gpu_sieve_v8_masks.py', 'sieve/es_word_masks.py',
             'harness/run_k_masks.py', 'ref/erdos_ref.py')
...
def survivor_hash(ns: list[int]) -> str:
...
def atomic_write(path: str, obj: dict) -> None:
    ...
    with open(tmp, "w") as f:
        ...
        os.fsync(f.fileno())
    os.replace(tmp, path)
    parent = os.open(os.path.dirname(os.path.abspath(path)), os.O_DIRECTORY)
    try:
        os.fsync(parent)
    finally:
        os.close(parent)
...
def run(k: int, ckpt_path: str, tun: dict[str, Any] | None, wheel: Any, max_blocks: int | None, timeout: float | None, log_every: float,
        ckpt_every: float, min_free_mib: int, resume: bool, start_block: int | None = None, max_launches: int | None = None) -> dict[str, Any]:
    ...
    engine = v5.__name__
    state: dict[str, Any] = {"engine_revision": engine_revision(), "k": k, "engine": engine, "wheel": wheel_json, "M": str(plan.M), "R": plan.R,
    ...
    if resume and os.path.exists(ckpt_path):
        ...
        if (old.get("k") == k and old.get("wheel") == wheel_json and old.get("R") == plan.R
                and old.get("engine") == engine and old.get("M") == str(plan.M)
                and old.get("engine_revision") == engine_revision()):
            state = old
            ...
            raise RuntimeError("Checkpoint plan/engine revision mismatch; explicit qualified migration required")

    if start_block is not None and state["t"] == 0 and state["idx_done"] == 0:
        state["t"] = start_block
        print(f"# START-BLOCK k={k} pre-scanning from block t={start_block} (max_blocks={max_blocks})", file=sys.stderr, flush=True)
    ...
    assert 0 <= state['unit_done'] <= units_total
    assert state['idx_done'] == state['unit_done'] * r0
    assert state['residues_checked'] >= state['idx_done']
    assert len(state['survivor_idx']) == len(set(state['survivor_idx']))
    assert all(0 <= i < state['idx_done'] for i in state['survivor_idx'])
    state['mask_mode'] = getattr(sieve, 'mask_mode', 'scalar-v8')
    ...
    while max_blocks is None or state["t"] < max_blocks:
        ...
        while state["unit_done"] < units_total:
            if STOP_REQUESTED or (max_launches is not None and sieve.launches >= max_launches) or (timeout and time.time() - t_start > timeout):
                state["residues_checked"] = sieve.residues_checked
                state["wall_accum"] = state.get("wall_accum", 0.0) + (time.time() - t0)
                save()
                return {"k": k, "status": "PAUSED" if STOP_REQUESTED or max_launches is not None else "TIMEOUT", "block": t, "idx_done": state["idx_done"],
                        ...
                        "wall": state["wall_accum"], "wall_leg": time.time() - t0,
                        "restarts": state.get("restarts", 0), "checkpoint": ckpt_path}
        ...
        if bad:
            ...
            return {"k": k, "status": "DISAGREEMENT", "block": t,
                    ...
                    "wall": state.get("wall_accum", 0.0) + (time.time() - t0),
...
def main() -> None:
    ...
    ap.add_argument("--start-block", type=int, default=None,
                    help="fresh runs begin at this block t (parallel future-block pre-scan)")
    ...
    res = run(a.k, ckpt, a.tune, wheel, a.max_blocks, a.timeout, a.log_every,
              a.ckpt_every, a.min_free_mib, not a.no_resume, a.start_block, a.max_launches)
    ...
    res['engine_revision'] = engine_revision()
    print(json.dumps(res, default=str), flush=True)
    if res.get('status') in ('DISAGREEMENT', 'SKIPPED_LOW_VRAM'):
        raise SystemExit(2)
