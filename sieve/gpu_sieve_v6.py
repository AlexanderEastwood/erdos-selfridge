...
def gen_kernel_src(radix: list[int], res_off: list[int], M: int,
    ...
    """Strip-mined kernel. radix[0] is the largest ring and is swept inside the thread, so its
    reconstruction cost is one 128-bit add per residue instead of len(radix).  All divisors are
    compile-time literals and both loops are fully unrolled by NVRTC."""
    M_lo, M_hi = M & ((1 << 64) - 1), M >> 64
    # degenerate case: no wheel at all (k=1 has no primes <= k). Then a block is the single
    # residue 0 and there is no inner ring to sweep; keep the loop shape with r0 = 1.
    inner = bool(radix)
    ...
    # outer rings 1..w-1: oidx decomposes over radix[1:], same least-significant-first order
    outer = list(zip(radix, res_off))[1:]
    for j, (r, off) in enumerate(outer, start=1):
        if r == 1:
    ...
    for j, (q, toff) in enumerate(zip(rest_q, rest_taboff)):
        ...
        L.append('            if (!ok) continue;')   # continues the d0 sweep -- next residue
    L.append('            {')
