...
TUNABLES = {
    ...
    "target_g": 0,          # v11: if > 0, size the wheel to a KNOWN g instead of to E[g].
                            # Ported verbatim from v10's planner onto v8's KERNEL. v10 shares
                            # v9's kernel, so a v10 confirmation of a v9 primary varies only the
                            # wheel and cannot catch a kernel bug. v11 gives the same g-sizing
                            # saving while keeping the confirmation kernel-independent of v9.
                            # Only legitimate when g is already known -- i.e. CONFIRMATION runs.
    "smem_budget_words": 11264,  # v8: u32 words of the packed table staged in shared memory.
                                 # 11264 words = 44 KB, kept under the 48 KB per-block static
                                 # __shared__ limit with margin; above this the kernel falls
                                 # back to reading the (still bit-packed) table from global.
}
...
def plan_wheel(k: int, ps: list[int], log2E: float, tun: dict) -> list[tuple[int, int]]:
    ...
    if tun.get("planner", "truecost") == "knapsack":
        ...
        if not feasible[best_c]:
            best_c = 0
    elif int(tun.get("target_g", 0)) > 0:
        # v11: g is KNOWN (confirmation run). Cost of covering [0, g) is exactly ceil(g/N) * R.
        # Identical to v10's branch; grafted onto v8's kernel so the confirmation stays
        # kernel-independent of a v9 primary.
        tg = int(tun["target_g"])
        log2g = math.log2(tg)
        lg_ratio = log2g - c_arr / scale                  # log2(g/N)
        with np.errstate(over="ignore", invalid="ignore"):
            nblocks = np.where(lg_ratio <= 0.0, 1.0,
                               np.ceil(np.exp2(np.clip(lg_ratio, -200.0, 200.0))))
        with np.errstate(divide="ignore", invalid="ignore"):
            cost = np.where(feasible & np.isfinite(nblocks), dp + np.log2(nblocks), np.inf)
...
def gen_kernel_src(radix: list[int], res_off: list[int], M: int,
                   rest_q: list[int], rest_taboff: list[int], n_fast: int = 4,
                   smem_words: int = 0) -> str:
    """Strip-mined kernel (v6), first `n_fast` filters carried incrementally in 32-bit (v7),
    filter tables bit-packed one bit per residue and the hot prefix staged in shared memory
    (v8).  All divisors and table offsets are compile-time literals.

    `rest_taboff` is now a WORD offset into the packed table, not a byte offset.  When
    smem_words > 0 the first n_fast tables (a contiguous prefix of the packed array, since
    the fast filters are the first n_fast entries of P.rest) are copied into shared memory
    once per thread block and read from there."""
    ...
    L.append('              const unsigned int* __restrict__ allowed,')
    L.append('              unsigned long long* __restrict__ out, unsigned int* __restrict__ out_n,')
    L.append('              const unsigned int out_cap)')
    L.append('{')
    if smem_words > 0:
        L.append(f'    __shared__ unsigned int sm[{smem_words}u];')
        L.append(f'    for (unsigned int z = threadIdx.x; z < {smem_words}u; z += blockDim.x)')
        L.append('        sm[z] = allowed[z];')
    ...
    for j in range(n_fast):
        ...
        L.append(f'              if (m >= {q}u) m -= {q}u;')
        src = 'sm' if smem_words > 0 else 'allowed'
        L.append(f'              if (!(({src}[{toff}u + (m >> 5)] >> (m & 31u)) & 1u)) ok = false; }}')
    ...
    for j in range(n_fast, len(rest_q)):
        ...
        L.append(f'              if (!((allowed[{toff}u + ((unsigned int)m >> 5)] '
                 f'>> ((unsigned int)m & 31u)) & 1u)) ok = false; }}')
        L.append('            if (!ok) continue;')
    L.append('            {')
...
class GpuSieve:
    def __init__(self, plan: Plan):
        ...
        # v8: one BIT per residue instead of one byte.  Each table is padded up to a whole
        # number of u32 words so every table starts word-aligned and `toff` is a word offset.
        self.rest_q, self.rest_Mmodq, toff = [], [], []
        words: list[np.ndarray] = []
        nw = 0
        for p in P.rest:
            q, res = P.info[p]
            self.rest_q.append(q); self.rest_Mmodq.append(P.M % q); toff.append(nw)
            w = np.zeros((q + 31) // 32, dtype=np.uint32)
            for r in res:
                w[r >> 5] |= np.uint32(1) << np.uint32(r & 31)
            words.append(w); nw += w.size
        self.nrest = len(P.rest)
        packed = np.concatenate(words) if words else np.zeros(1, dtype=np.uint32)
        self.allowed = cp.asarray(packed)
        self.packed_words = int(packed.size)
        self.toff_words = toff
        ...
        # The fast filters are the first n_fast entries of P.rest, so their packed tables are a
        # contiguous prefix: staging words [0, smem_words) covers exactly them.
        need = toff[self.n_fast] if self.n_fast and self.n_fast < len(toff) else (
            self.packed_words if self.n_fast else 0)
        budget = int(P.tun.get("smem_budget_words", 12288))
        self.smem_words = int(need) if 0 < need <= budget else 0
        self.smem_bytes = self.smem_words * 4
