...
TUNABLES = {
    ...
    "maxrregcount": "auto",  # v9: cap registers/thread to raise occupancy.
                            # v7 lets the compiler choose and is register-limited: 62 regs at
                            # k=379 -> 4 blocks/SM -> 66.7% occupancy, no spills. Capping to 42
                            # reaches 6 blocks/SM (100%) and measured +7.0% at k=381 -- but a
                            # FIXED cap of 42 spills to local memory at 7 of 17 k tested
                            # (20, 60, 140, 200, 280, 382, 400), and a spill can cost more than
                            # the occupancy gain. "auto" therefore walks a ladder of caps from
                            # the most aggressive down and takes the first that does NOT spill,
                            # falling back to the compiler's own choice. It can never spill and
                            # never has worse occupancy than v7.
                            # 0 = off (v7 behaviour); an int = that fixed cap, spills allowed.
}

# Register caps that hit each occupancy tier at 256 threads/block on a 65536-reg SM:
#   42 -> 6 blocks/SM (100%),  51 -> 5 (83.3%),  64 -> 4 (66.7%, v7's natural point)
REGCAP_LADDER = (42, 51)
...
class GpuSieve:
    def __init__(self, plan: Plan):
        ...
        t_compile = time.time()
        _mrc = P.tun.get("maxrregcount", 0)
        if _mrc == "auto":
            # Try each cap in turn; accept the first that compiles WITHOUT spilling to local
            # memory. Costs a few NVRTC compiles once per process (seconds against hours of
            # scanning) and is strictly safe: worst case it lands on the uncapped kernel.
            self.kernel, self.regcap = None, 0
            for cap in REGCAP_LADDER:
                cand = cp.RawKernel(self.src, "es_sieve",
                                    options=("-std=c++17", f"-maxrregcount={cap}"))
                cand.compile()
                if cand.local_size_bytes == 0:
                    self.kernel, self.regcap = cand, cap
                    break
            ...
            _mrc = int(_mrc)
            self.regcap = _mrc
            _opts = ["-std=c++17"] + ([f"-maxrregcount={_mrc}"] if _mrc else [])
            self.kernel = cp.RawKernel(self.src, "es_sieve", options=tuple(_opts))
        self.kernel.compile()
